...
3. **Ablations** on aggregation, cluster-selection strategy, and number of clusters that overturn the design we started with: the hand-designed composite "anomaly-likeness" selector we expected to help was the worst of three; cluster-selection strategy is modality-dependent, and a smallest+random ensemble is the robust default.
4. **Panel robustness** and **stability** analyses: leave-one-detector-out never materially changes the ranking, and regret is stable across seeds (std ≤ 0.017), so the effect is neither carried by a single detector nor an artifact of a lucky seed.
...
<figure>
<img src="figures/fig4_regret.png" alt="Bar chart of regret@1 for ADRank versus random pick across four modalities">
<figcaption><b>Figure 4.</b> Detector-selection regret@1 (true-AUC gap between the best detector and the selector's pick; lower is better) for ADRank versus random pick, mean over five seeds with seed-standard-deviation error bars. ADRank cuts regret by 77–90% on tabular, image, and time-series data; on text the reduction is 21%.</figcaption>
</figure>
...
The takeaway: given any encoder that produces a fixed-length vector per input, ADRank selects a near-best detector on tabular, image, and time-series data; text is harder and only marginally beats random, which regret reports honestly where thresholded rank-correlation does not.
...
**When ADRank helps and when it does not.** The method selects a near-best detector on tabular, image, and time-series data (regret@1 ≤ 0.021, 77–90% below random). On text embeddings it only marginally beats random pick, and we report that plainly. Whether a label-free confidence signal can flag the weak case a priori is an open question: the pseudo-AUC spread across detectors, the natural candidate, correlates with true-AUC spread at only ρ = 0.14, so it is not a reliable gate; the bootstrap stability of the ranking itself is a more promising signal we leave to future work.
...
The pseudo-anomaly analogy is imperfect and its quality is modality-dependent. On text embeddings ADRank only marginally beats random selection (21% regret reduction), so it should not be relied on for detector choice on BERT-encoded text without a labeled check. The method requires an expressive fixed-length representation: on high-dimensional deep embeddings the `smallest`-cluster selection degrades and the smallest+random ensemble is needed, and on time-series a richer window descriptor is needed than a handful of summary statistics. The evaluation uses fixed default hyperparameters per detector; combining ADRank with per-detector hyperparameter search is left to future work. Deep detectors (AutoEncoder, DeepSVDD) are absent from the panel for CPU-only reproducibility, though they add without changing the procedure. Confidence intervals are derived from five seeds; regret standard deviations are already small (≤ 0.017), so more seeds would tighten them marginally without changing the ordering of modalities.

## 8. Conclusion

Clusters of the unlabeled training distribution behave, for the purpose of ranking anomaly detectors, close enough to real anomalies. Averaging pseudo-AUC over cluster-holdout draws and picking the top-ranked detector cuts selection regret versus random by 77% on tabular ADBench data, 81% on image embeddings, and 90% on time-series, with a seed standard deviation at or below 0.004; on text embeddings the reduction is a modest 21%, which we report as the method's honest limit. Evaluated by regret, a threshold-free and decision-relevant loss, the result is stable where thresholded rank-correlation is not. The design is simple: cluster once, hold out a smallest+random ensemble of cluster subsets, average AUC, rank. It gives practitioners a cheap, unsupervised, panel-robust selector for an anomaly detector, together with an evaluation protocol that reports its reliability honestly.
