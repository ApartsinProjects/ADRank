...
CACHE = os.path.join(ROOT, "data", "oddbench")
TRIV_PCT, MIN_HARD = 99.0, 20
# FIX 6 (audit): this arm never received the base-rate cap that the OvrBench arm has. 25
# scored datasets exceed 0.25 and 9 reached the include list with TEST base rates 0.35-0.87
# (ChainConfig 0.87, BioExperiment 0.68), where ap_norm = (ap - base)/(1 - base) is close to
# degenerate. The second cap bounds the TEST rate that ap_norm actually divides by, since
# the test set is held-out normals plus ALL hard anomalies.
MAX_DATASET_RATE = 0.25
MAX_TEST_RATE = 0.25
SEEDS = [0, 1, 2]
MAX_N, MAX_D_LINRES = 5000, 120
OUT = r"E:\tmp\claude\E--Projects-Submitted-ADRank\236d6247-42ad-4e62-9828-db625dfa055d\hadb_oddbench.csv"
...
N_MAX = int(sys.argv[1]) if len(sys.argv) > 1 else 10 ** 9   # limit, for smoke tests
files = sorted(f for f in os.listdir(CACHE) if f.endswith(".npz"))[:N_MAX]
...
for i, fn in enumerate(files, 1):
    ...
    try:
        ...
        meta.append(dict(dataset=name, corpus="oddbench", status="dropped",
                         reason=f"load_{type(e).__name__}")); continue
    X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
    n_raw = len(X)
    # FIX 5 (audit): keeping the first occurrence of a duplicate feature row silently kept
    # whichever label came first when identical rows carried BOTH. 22 scored datasets kept at
    # least one anomaly identical to a normal row (GlobalTerrorismAttack 234 of 682 hard).
    # Undetectable in principle, so they pass the triviality filter by construction and enter
    # the benchmark as label noise. Drop the whole conflicting group.
    ...
    rec = dict(dataset=name, corpus="oddbench", n_raw=n_raw, n=len(X), d=X.shape[1],
               dup_frac=round(float(dup), 4), n_mixed_dup_rows=n_mixed_rows,
               n_anom=int(y.sum()), rate=round(float(y.mean()), 5))
    if len(X) < 200 or y.sum() < 10 or y.sum() == len(y):
        rec.update(status="dropped", reason="too_small_or_degenerate")
    ...
    # recorded "scored" with zero result rows (GlobalSharkAttacks did exactly that).
