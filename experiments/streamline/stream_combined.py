...
for _, r in f.iterrows():
    ...
    cn, ca = np.maximum(so_n, sp_n), np.maximum(so_a, sp_a); thr = np.quantile(cn, 1 - Q)
    hard = Xa[ca <= thr]
    rec = {"corpus": r.corpus, "dataset": r.dataset, "knn_orig": sus.get((r.corpus, r.dataset), np.nan),
           ...
           "catch_comb": round(float((ca > thr).mean()), 2), "fp_comb": round(float((cn > thr).mean()), 2),
           "nhard_comb": len(hard)}
    if len(hard) >= 20:
...
print(f"=== UNIFIED rule (original UNION PCA, tail+histogram) TEST ({len(df)} datasets, @5% FP) ===")
